...
# Serviços críticos para monitorar (adicione os do seu ambiente)
$CriticalServices = @(
...
function Test-ADHealth {
    ...
    try {
        ...
        $results += [PSCustomObject]@{
            Verificação = "Usuários ativos no AD"
            Status      = "OK"
        ...
        $results += [PSCustomObject]@{
            Verificação = "Módulo ActiveDirectory"
            Status      = "ALERTA"
            Detalhe     = "Módulo RSAT não instalado ou sem permissão de consulta"
        }
    }
    return $results
}
...
$ServiceResults = $CriticalServices | ForEach-Object {
    ...
    [PSCustomObject]@{
        Serviço = if ($svc) { $svc.DisplayName } else { $_ }
        Nome    = $_
...
# Eventos de Erro (últimas 24h — apenas críticos)
...
$SvcRows = ($ServiceResults | ForEach-Object {
    "<tr><td>$($_.Serviço)</td><td>$(Get-StatusBadge $_.Status)</td><td>$($_.Estado)</td></tr>"
}) -join "`n"
...
if ($ADResults.Count -gt 0) {
    $ADRows = ($ADResults | ForEach-Object {
        "<tr><td>$($_.Verificação)</td><td>$(Get-StatusBadge $_.Status)</td><td>$($_.Detalhe)</td></tr>"
    }) -join "`n"
}
